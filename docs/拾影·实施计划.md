...
├── build.sh                         # 跨平台打包脚本
├── 拾影.spec                        # PyInstaller 打包配置
├── .gitignore
├── docs/
│   ├── 拾影·朋友圈智能美图筛选系统设计文档.md  # 需求与架构设计
│   ├── 拾影·开发方案.md                       # 开发策略与阶段划分
│   └── 拾影·实施计划.md                       # TDD 实施计划（本文档）
├── ui/
│   ├── __init__.py
│   ├── main_window.py               # 主窗口（集成所有组件）
│   ├── sidebar.py                   # 侧边栏（模式/阈值/配置/统计）
...
└── tests/
    ├── __init__.py
    ├── test_image_utils.py          # 图片工具测试（4 个）
    ├── test_algorithm_filter.py     # 算法筛选测试（6 个）
    ├── test_scorer.py               # 评分接口测试（3 个）
    └── test_exporter.py             # 导出模块测试（3 个）
```
